...
# LABADMIN CONTROLLER
#         FILE: labadmin
#        USAGE: labadmin [-c file_config] [-i iface] [-r range] [-a script] [-1 value1] [-2 value2]...
#
#  DESCRIPTION: Admin remotely hosts in a computer lab environment
#
#       AUTHOR: Leonardo Marco, leonardojose.marco@murciaeduca.es
...
#  LOAD MODULES
#===============================================================================
source "$(dirname "$(readlink -f "$0")")/modules/labadmin_lib-admin"			# LABADMIN ADMIN VARIABLES AND FUNCTIONS
source "${modules_path}/labadmin_lib"											# LABADMIN GLOBAL VARIABLES AND FUNCTIONS
source "${modules_path}/labadmin_ip"											# LABADMIN IP MODULE
source "${modules_path}/labadmin_map"											# LABADMIN MAP MODULE
source "${modules_path}/labadmin_script"										# LABADMIN SCRIPT MODULE
source "${modules_path}/labadmin_install"										# LABADMIN INSTALL MODULE
...
unset ipadmin			# Admin IP
unset config_id			# Config file id (filename without extension)
unset config_file		# Config file with path
...
function help() {
   ...
   '"${S_B}$(basename $0)${S_R}"' [-c '"${S_U}"'file_config'"${S_R}"'] [-i '"${S_U}"'face'"${S_R}"'] [-r '"${S_U}"'range'"${S_R}"'] [-a '"${S_U}"'action'"${S_R}"'] [-1|-2|-3|-4|-5|-6|-7|-8|-9 '"${S_U}"'param'"${S_R}"'] [-W|-L] [-F]

  OPTIONS
   '"${S_B}"'-c '"${S_U}"'file_config'"${S_R}"'
\tConfig filename to use
\tConfig files must be placed in '"${S_B}"'configs/'"${S_R}"' directory
\tWhen not specified '"${S_B}"'default'"${S_R}"' file is used.

   '"${S_B}"'-i '"${S_U}"'iface'"${S_R}"'
\tNetwork interface to use. Overrides iface variable in config file.

   '"${S_B}"'-a '"${S_U}"'n_action'"${S_R}"'
...
\tAction parameter value to exec.
   ...
   '"${S_B}"'-W '"${S_R}"'
\tForce Windows WINRM server mode. Overrides srvmode variable in config file.

   '"${S_B}"'-L '"${S_R}"'
\tForce Linux SSH server mode. Overrides srvmode variable in config file.

  SAMPLES
    '"${S_B}$(basename $0)${S_R}"' -c a11 -i eth0
    '"${S_B}$(basename $0)${S_R}"' -r 5,12,15-20 -a exec -1 c -2 "du -sh /home/*"
    '"${S_B}$(basename $0)${S_R}"' -r all -a "update&" -1 y -2 y
'
	exit $1
}
...
function check_params() {
	while getopts ":hc:i:r:a:1:2:3:4:5:6:7:8:9:SWLF" o
	do
		case $o in
			h) hparam=1; help 0 ;;
			c) # -c:CONFIG
				cparam=1
				config_id="$OPTARG"
				if [ "${config_id:0:1}" = "-" ]; then
					echo "Param -c needs a valid config ID" 1>&2
					echo "Available config IDs: $(ls $configs_path |  sed -z 's/\n/, /g' | sed 's/, $//g')" 1>&2
					exit 1
				fi
...
#=== FUNCTION ==================================================================
#        NAME: initialize
# DESCRIPTION: read parameters and initialize configs
#===============================================================================
function initialize() {
	...
	##### LOAD CONFIG FILE
	[ ! "$noninteractive" ] && echo "Loading config files"
	if [ ! "$config_id" ]; then
		config_id="default"
		config_file="$configs_path/default"
	fi
	if [ ! -r "$config_file" ]; then
		labadmin_error "$(echo "Can't access config file $config_file"|tr -s "/")"
		echo "Available configs to use:"
		find "$configs_path" -type f | sed 's\'"${configs_path}"'\ \g'
		exit 1
	fi
	...
	! [ "$ipadmin" ] && labadmin_error "Cant get IP from iface $iface\nPlease check iface config in $config_file or use -i param" 1
	...
	[ ! "$noninteractive" ] && echo "Checking module dependencies"
