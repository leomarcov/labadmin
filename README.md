...
Labadmin is a Linux Bash script capable to remotely admin a set of hosts (Linux or Windows). It's an approach similar to Ansible, but Labadmin is focused to work in educational environment with hosts in a computer laboratory. 
...
## Controlled hosts install in Linux
...
## Controlled hosts install in Windows
...
You can select lab config loaded and mode with parameters: ``-l <lab_file>`` and ``-L`` or ``-W``. 
```bash
